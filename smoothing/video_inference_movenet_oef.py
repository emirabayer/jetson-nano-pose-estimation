...
OUTPUT_VIDEO_PATH = 'output_movenet_smooth.mp4'
...
if __name__ == '__main__':
    ...
    filter_config = {
        'freq': video_fps,
        'min_cutoff': 0.3,   # Aggressive smoothing, used to be 0.4
        'beta': 0.1,      # Reduced sensitivity to speed, used to be 0.05
        'd_cutoff': 1.0
    }
    ...
    while cap.isOpened():
        ...
        # Swap the columns from (y, x) to (x, y) for drawing
        smoothed_keypoints_for_drawing = smoothed_keypoints[:, [1, 0]]

        # Draw the correctly ordered keypoints on the original frame
        frame_with_keypoints = draw_keypoints_on_frame(frame.copy(), smoothed_keypoints_for_drawing)
